<x-app-layout>    
    <div class="flex flex-col items-center py-10">
        <h1 class="text-xl font-bold py-2">Languages</h1>
        ...
        <table class="self-center shadow-lg bg-white">
            ...
            <tbody>
                @foreach ($languages as $language)
                    <tr>
                        <td class="border px-8 py-4">{{$language->id}}</td>
                        <td class="border px-8 py-4">{{$language->name}}</td>
                        <td>
                        ...
                        <td>                        
                            <form class="form-language-admin" action="{{ route('admin.languages.destroy',$language)}}" method="POST">
                                @csrf
                                @method('delete')
...
<script>
    $(function () {
        $('.form-language-admin').submit(function (e) {
            e.preventDefault();
            //SWAL
        ...
        swalCustom.fire({
            icon: 'question',
            html: '<strong class="html-message">Are you sure you want to delete the language?</strong>',
            background: '#e9e9e8',
