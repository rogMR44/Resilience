    ...
    <div class="flex flex-col items-center py-10">
        ...
        <table class="self-center shadow-lg bg-white">
            ...
            <tbody>
                @foreach ($tags as $tag)
                    <tr>
                        <td class="border px-8 py-4">{{ $tag->id}}</td>
                        ...
                        <td>                        
                            <form class="form-tag-admin" action="{{ route('admin.tags.destroy',$tag)}}" method="POST">
                                @csrf
                                @method('delete')
...
<script>
    $(function () {
        $('.form-tag-admin').submit(function (e) {
            e.preventDefault();
            //SWAL
        ...
        swalCustom.fire({
            icon: 'question',
            html: '<strong class="html-message">Are you sure you want to delete the tag?</strong>',
            background: '#e9e9e8',
